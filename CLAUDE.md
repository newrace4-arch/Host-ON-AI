...
8. **커밋은 그 자리에서, 체크리스트 갱신은 주 2회 일괄 (9/5 확정 →
   ...
   - **체크리스트 갱신은 주 2회 일괄로 처리한다 — 화요일과 토요일.**
   - **그 외의 날에는 엑셀을 열지 않는다. 읽기도 하지 않는다.**
     그날 할 일은 **전날 devlog 5절**에서 본다.
   - 첫 적용: **9/12(토).** 9/11~9/12 완료분을 함께 처리한다.

   > **왜 바꿨는가(9/10)**: 태스크 하나마다 8-2 전체(백업 · `--selftest` ·
   > 변경 명세 · 6단계 검증 · 커밋)를 돌리면 **셀 몇 개를 바꾸는 데
   > 20~30분**이 든다. **8-2 비용의 대부분은 태스크 수와 무관한 고정
   > 비용**이라 묶을수록 유리하다. 9/10에 실제로 **엑셀 작업이 다섯 번**
   > 있었고 개발 시간을 잠식했다.
   >
   > 체크리스트는 **제출물이 아니고 계획 대비 진척을 보는 도구**다.
   > 그날그날의 기록은 **devlog와 git log**가 담당한다.
   >
