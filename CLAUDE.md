...
8. **작업 완료 = 커밋 + 체크리스트 갱신을 항상 함께 묶는다 (9/5
   ...
   **같은 작업 안에서** 다음을 함께 처리한다:
   ...
   - **8-2. 체크리스트 엑셀 수정 절차 (9/6 확정).** 아래 6단계를
     순서대로 지킨다. 이 절차는 위 8번의 "작업 완료 시 자동 갱신"과
     ...
     4. 저장 **전후로 항상** 차트·이미지·피벗(0개 기준)·조건부서식·
        데이터유효성·수식 개수를 비교 검증하고 결과를 보고한다.
...
- `docs/api_contract.md` — API 엔드포인트 목록·요청/응답 스펙(v2.2)
