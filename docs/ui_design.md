...
| EMPTY | "이 기간에 예약이 없습니다" + "예약 추가" 버튼(빈 칸 클릭 유도) |
...
| EMPTY | "지금 처리할 일이 없습니다" — 정상 상태임을 명시(경고처럼 보이지 않게) |
...
| EMPTY | "등록된 지식이 없습니다. AI가 답변할 근거가 없으면 응대 품질이 떨어집니다" + 첫 항목 등록 유도 |
