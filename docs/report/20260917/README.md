...
| 02 | `02_calendar_property.png` | **독채(PROPERTY 단위) — 5건.** 같은 화면이 판매단위에 따라 **다른 행 구성**을 그린다. *"요금 미설정"*(`net_amount` null)과 `CANCELLED` 표시. **04의 턴오버를 만든 두 건이 여기 보인다**(아래 「쓰기 전에 알아야 할 것」) |
...
| 06 | `06_pytest.png` | **회귀 315 passed** — PowerShell 실제 출력(`58.32s`) |
...
**그 턴오버를 만든 예약이 `02`에 보인다** — 가운데 두 줄이다:

    2026-09-12 ~ 2026-09-14  한지우   ← 오늘(9/14) 체크아웃
    2026-09-14 ~ 2026-09-15  오서연   ← 오늘(9/14) 체크인

둘의 판매단위가 `(room_id=NULL, bed_id=NULL)`로 **같아서** 턴오버로 잡힌다.
`02`와 `04`를 나란히 놓고 **교차 검증할 수 있다.**

> ✅ **시드 불일치는 해소됐다.** `02`를 처음 찍었을 때(15:59)는 이 두 건이
> 아직 없어 독채가 **3건**으로 나왔고 `04`의 턴오버와 맞지 않았다.
> **18:25에 다시 찍어 5건으로 교체했다.**
