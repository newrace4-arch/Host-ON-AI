...
>
> ⏰ **마감: 10/9까지 반드시 생성한다.** 지금 2절이 v1.3 상태로 남아 있는
> 것은 위험하지 않지만, **10/10 제출물(설명문서 + 소스코드 ZIP)에 낡은
> ERD가 들어가면 회수할 수 없다.** 스키마는 **10/8 24시 Feature Freeze**로
> 고정되므로 그 직후가 생성 적기다.
>
> | 시점 | 할 일 |
> |---|---|
> | 10/8 Feature Freeze 직후 | 마지막 마이그레이션이 적용된 DB에서 2절 생성 |
> | **10/9** | 생성 결과를 1·3절과 대조하고 커밋 — **이 날을 넘기지 않는다** |
> | 10/10 | 제출. 이때는 ERD를 손대지 않는다 |
>
> **자동 생성기를 못 만들면 그날 손으로라도 2절을 v1.4에 맞춘다.** 도구가
> 없다는 이유로 낡은 ERD를 제출하지 않는다 — 이 마감은 도구가 아니라
> **문서의 정확성**에 걸린 것이다.
>
